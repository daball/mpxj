 ...
 */
class TaskPercentCompleteTest {
    /**
     * Test to validate Primavera percent complete values.
     */
    @Test
    @Throws(MPXJException::class)
    fun testPrimaveraPercentComplete() {
        val testDataDir = File(MpxjTestData.filePath("generated/task-percentcomplete"))
        var project = PrimaveraXERFileReader().read(File(testDataDir, "percent-complete-8.4.xer"))
        testPrimaveraPercentComplete(project, 2)

        project = PrimaveraPMFileReader().read(File(testDataDir, "percent-complete-8.4.pmxml"))
     ...
     */
    private fun testPrimaveraPercentComplete(project: ProjectFile, startTaskID: Int) {
     ...
     */
    private fun testPercentComplete(project: ProjectFile, taskID: Int, expectedTaskName: String, expectedPercentCompleteValue: Int) {
        val task = project.getTaskByID(Integer.valueOf(taskID))
        assertEquals(expectedTaskName, task.name)
        assertEquals(expectedPercentCompleteValue, task.percentageComplete.intValue())
    }
}