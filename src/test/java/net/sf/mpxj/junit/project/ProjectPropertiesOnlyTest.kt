 ...
 */
class ProjectPropertiesOnlyTest {
    /**
     ...
     */
    @Throws(MPXJException::class)
    private fun testPropertiesOnly(file: File) {
        val reader = MPPReader()
        var project = reader.read(file)
        assertTrue(project.getTasks().size() > 0)
        assertEquals("Project User", project.projectProperties.author)
