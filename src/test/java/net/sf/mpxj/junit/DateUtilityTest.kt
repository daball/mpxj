...
class DateUtilityTest {
    ...
    fun testCompare() {
        ...
        val date1 = df.parse("15/01/2014")
        val date2 = df.parse("20/01/2014")

        assertEquals(-1, date1.compareTo(date2))
        assertEquals(1, date2.compareTo(date1))
        assertEquals(0, date1.compareTo(date1))

        assertEquals(-1, DateHelper.compare(date1, date2).toLong())
        assertEquals(1, DateHelper.compare(date2, date1).toLong())
        assertEquals(0, DateHelper.compare(date1, date1).toLong())
    }
