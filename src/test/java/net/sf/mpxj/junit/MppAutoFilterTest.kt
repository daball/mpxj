...
class MppAutoFilterTest {
    ...
    fun testMpp9Filters() {
        val mpp = MPPReader().read(MpxjTestData.filePath("mpp9autofilter.mpp"))
        testFilters(mpp)
    }
    ...
    fun testMpp9FiltersFrom12() {
        val mpp = MPPReader().read(MpxjTestData.filePath("mpp9autofilter-from12.mpp"))
        testFilters(mpp)
    }
    ...
    fun testMpp9FiltersFrom14() {
        val mpp = MPPReader().read(MpxjTestData.filePath("mpp9autofilter-from14.mpp"))
        testFilters(mpp)
    }
    ...
    fun testMpp12Filters() {
        val mpp = MPPReader().read(MpxjTestData.filePath("mpp12autofilter.mpp"))
        testFilters(mpp)
    }
    ...
    fun testMpp12FiltersFrom14() {
        val mpp = MPPReader().read(MpxjTestData.filePath("mpp12autofilter-from14.mpp"))
        testFilters(mpp)
    }
    ...
    fun testMpp14Filters() {
        val mpp = MPPReader().read(MpxjTestData.filePath("mpp14autofilter.mpp"))
        testFilters(mpp)
    }
    ...
    private fun testFilters(mpp: ProjectFile) {
        ...
        val view = mpp.views.get(0) as GanttChartView
        assertEquals("Gantt Chart", view.name)

        //
        // Test all data types
        //
        var filter = view.getAutoFilterByType(TaskField.DURATION)
