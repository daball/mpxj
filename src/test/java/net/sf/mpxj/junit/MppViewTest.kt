...
class MppViewTest {
    ...
    fun testMpp9View() {
        val mpp = MPPReader().read(MpxjTestData.filePath("mpp9resource.mpp"))
        testViews(mpp)
    }
    ...
    @Throws(Exception::class)
    fun testMpp12View() {
        val mpp = MPPReader().read(MpxjTestData.filePath("mpp12resource.mpp"))
        testViews(mpp)
    }
    ...
    @Test
    @Throws(Exception::class)
    fun testMpp12ViewFrom14() {
        val mpp = MPPReader().read(MpxjTestData.filePath("mpp12resource-from14.mpp"))
        testViews(mpp)
    }
    ...
    @Throws(Exception::class)
    fun testMpp14View() {
        val mpp = MPPReader().read(MpxjTestData.filePath("mpp14resource.mpp"))
        testViews(mpp)
    }
     ...
     */
    private fun testViews(mpp: ProjectFile) {
        val views = mpp.views

        // not sure what order MPP12 will store the views in,
        // so make a Set to check against when done reading in the views
        val setViewNames = HashSet<String>()

        for (viewNum in 0 until views.size()) {
            // View Names
            val view = views.get(viewNum)
            val viewName = view.getName()
            setViewNames.add(viewName)
            var table: Table? = null

            if (view is GanttChartView) {
                val view9 = view as GanttChartView
                if (null != view9.table) {
                    table = view9.table

                    val viewType = view.getType()
                    assertEquals(ViewType.GANTT_CHART, viewType)

                    // verify all columns
                    val cols = table!!.columns
                    val setColumnNames = HashSet<String>()
                    for (n in 0 until cols.size()) {
                        val col = cols.get(n)
                        setColumnNames.add(col.getTitle())
                        val width = col.getWidth()
                        assertTrue(width > 0)
                    }
