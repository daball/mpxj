...
class MppFilterTest {
    ...
    fun testMpp9Filters() {
        val mpp = MPPReader().read(MpxjTestData.filePath("mpp9filter.mpp"))
        executeTests(mpp)
    }
    ...
    @Test
    @Throws(Exception::class)
    fun testMpp12Filters() {
        val mpp = MPPReader().read(MpxjTestData.filePath("mpp12filter.mpp"))
        executeTests(mpp)
    }
    ...
    @Test
    @Throws(Exception::class)
    fun testMpp12FiltersFrom14() {
        val mpp = MPPReader().read(MpxjTestData.filePath("mpp12filter-from14.mpp"))
        executeTests(mpp)
    }
    ...
    @Test
    @Throws(Exception::class)
    fun testMpp14Filters() {
        val mpp = MPPReader().read(MpxjTestData.filePath("mpp14filter.mpp"))
        executeTests(mpp)
    }
     ...
     */
    private fun executeTests(mpp: ProjectFile) {
        testFilters(mpp)
     ...
     */
    private fun testFilters(mpp: ProjectFile) {
        //
        // Test all data types
        //
        val filters = mpp.filters

        var filter = filters.getFilterByName("Filter 1")
     ...
     * @param mpp project file
     */
    private fun testFilterEvaluation(mpp: ProjectFile) {
        ...
        val task6 = mpp.getTaskByID(Integer.valueOf(6))
        val task7 = mpp.getTaskByID(Integer.valueOf(7))
        ...
        filter = Filter()
        var criteria = GenericCriteria(properties)
        filter.setCriteria(criteria)
        criteria.leftValue = TaskField.DEADLINE
        criteria.operator = TestOperator.IS_ANY_VALUE
        assertTrue(filter.evaluate(task1, null))

        // Create and test a boolean filter
        filter = Filter()
        criteria = GenericCriteria(properties)
        filter.setCriteria(criteria)
        criteria.leftValue = TaskField.FLAG1
        criteria.operator = TestOperator.EQUALS
     ...
     * @param mpp project file
     */
    private fun testLogicalOperatorEvaluation(mpp: ProjectFile) {
        val task6 = mpp.getTaskByID(Integer.valueOf(6))
        val task7 = mpp.getTaskByID(Integer.valueOf(7))
        val filters = mpp.filters

        // Number1==13 && Number2==7
        var filter = filters.getFilterByName("Filter 19")
        assertFalse(filter.evaluate(task6, null))
        assertTrue(filter.evaluate(task7, null))
     ...
     * @param mpp project file
     */
    private fun testParameters(mpp: ProjectFile) {
        val filter = mpp.filters.getFilterByName("Filter 24")
        assertNotNull(filter)

        val prompts = filter.getPrompts()
     ...
     */
    @Test
    fun testNullValueTestOperators() {
        var operator = TestOperator.CONTAINS
        ...
        val allNull = arrayOf<Object>(null, null)
        val lhsNull = arrayOf<Object>(null, Integer.valueOf(10))
        val rhsNull = arrayOf<Object>(Integer.valueOf(1), null)
