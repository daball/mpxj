 ...
 */
internal class BtrieveDateColumn
/**
 * Constructor.
 *
...
(name: String, offset: Int) : AbstractColumn(name, offset) {
    ...
    override fun read(offset: Int, data: ByteArray): Date? {
        ...
        val i = offset + m_offset

        val day = data[i].toInt()
        val month = data[i + 1].toInt()
        val year = data[i + 2] and 0xff or (data[i + 3] and 0xff shl 8)
        if (year != 0) {
