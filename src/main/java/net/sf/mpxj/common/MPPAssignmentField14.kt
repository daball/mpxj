/*
 * file:       MPPAssignmentField14.java
 * author:     Jon Iles
 ...
 */
object MPPAssignmentField14 {

    val MAX_VALUE = 715

    private val FIELD_ARRAY = arrayOfNulls<AssignmentField>(MAX_VALUE)

    private val ID_ARRAY = IntArray(AssignmentField.MAX_VALUE)

    val ASSIGNMENT_FIELD_BASE = 0x0F400000
    /**
     ...
     */
    fun getInstance(value: Int): AssignmentField? {
        var result: AssignmentField? = null

        if (value >= 0 && value < FIELD_ARRAY.size) {
            result = FIELD_ARRAY[value]
        } else {
            if (value and 0x8000 != 0) {
                val baseValue = AssignmentField.ENTERPRISE_CUSTOM_FIELD1.getValue()
     ...
     * @return field ID
     */
    fun getID(value: AssignmentField): Int {
    ...
    init {
        ...
        for (loop in FIELD_ARRAY.indices) {
            val assignmentField = FIELD_ARRAY[loop]
