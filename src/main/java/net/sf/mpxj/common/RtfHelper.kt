...
object RtfHelper {
     ...
     */
    private fun isFormalRTF(text: String): Boolean {
    ...
    fun strip(text: String?): String? {
        ...
        if (text != null && !text.isEmpty()) {
            try {
                val formalRTF = isFormalRTF(text)
                val stc = StringTextConverter()
                stc.convert(RtfStringSource(text))
                result = stripExtraLineEnd(stc.text, formalRTF)
     ...
     */
    private fun stripExtraLineEnd(text: String, formalRTF: Boolean): String {
        var text = text
