 ...
 */
abstract class AbstractTimephasedWorkNormaliser : TimephasedWorkNormaliser {
    /**
     ...
     */
    @Override
    abstract override fun normalise(calendar: ProjectCalendar, list: LinkedList<TimephasedWork>)
     ...
     */
    protected fun mergeSameWork(list: LinkedList<TimephasedWork>) {
        ...
        for (assignment in list) {
            if (previousAssignment == null) {
                ...
                val previousAssignmentWork = previousAssignment!!.getAmountPerDay()
                ...
                if (NumberHelper.equals(previousAssignmentWork.getDuration(), assignmentWork.getDuration(), 0.01)) {
                    ...
                    var total = previousAssignment!!.getTotalAmount().getDuration()
                    total += assignmentWork.getDuration()
                    val totalWork = Duration.getInstance(total, TimeUnit.MINUTES)
     ...
     * @param list assignment data
     */
    protected fun convertToHours(list: LinkedList<TimephasedWork>) {
        for (assignment in list) {
            var totalWork = assignment.getTotalAmount()
            var workPerDay = assignment.getAmountPerDay()
