 ...
 */
open class MultiDateFormat
/**
 * Constructor. Optionally allows multiple patterns to be passed.
 *
 * @param patterns date format patterns
 */
(vararg patterns: String) : DateFormat() {

    protected var m_formats: Array<SimpleDateFormat>
     ...
     */
    @Override
    fun parse(str: String?, pos: ParsePosition): Date? {
        val result: Date?
     ...
     */
    protected open fun parseNonNullDate(str: String, pos: ParsePosition): Date? {
        var result: Date? = null
        for (index in m_formats.indices) {
     ...
     */
    @Override
    fun format(date: Date, toAppendTo: StringBuffer, fieldPosition: FieldPosition): StringBuffer {
