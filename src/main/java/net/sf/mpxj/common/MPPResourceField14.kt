 ...
 */
object MPPResourceField14 {

    private val MAX_VALUE = 809
    private val FIELD_ARRAY = arrayOfNulls<ResourceField>(MAX_VALUE)

    private val ID_ARRAY = IntArray(ResourceField.MAX_VALUE)
    /**
     ...
     */
    fun getInstance(value: Int): ResourceField? {
        ...
        if (value >= 0 && value < FIELD_ARRAY.size) {
            ...
            if (value and 0x8000 != 0) {
                val baseValue = ResourceField.ENTERPRISE_CUSTOM_FIELD1.value
                val id = baseValue + (value and 0xFFF)
     ...
     * @return field ID
     */
    fun getID(value: ResourceField): Int {
        return ID_ARRAY[value.value]
    }
    ...
    init {
        ...
        //FIELD_ARRAY[586] = ResourceField.ENTERPRISE_DEPARTMENT;
    ...
    init {
        ...
        for (loop in FIELD_ARRAY.indices) {
            val resourceField = FIELD_ARRAY[loop]
            if (resourceField != null) {
                ID_ARRAY[resourceField.value] = loop
            }
        }
    }
}
