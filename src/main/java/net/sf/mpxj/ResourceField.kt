 ...
 */
enum class ResourceField// Must always be last value
...
private constructor(private val m_dataType: DataType,
    ...
    val fieldTypeClass: FieldTypeClass
        @Override get() = FieldTypeClass.RESOURCE
    ...
    fun getName(locale: Locale): String? {
        val titles = LocaleData.getStringArray(locale, LocaleData.RESOURCE_COLUMNS)
    ...
    companion object {
         ...
         */
        fun getInstance(type: Int): ResourceField? {
        ...
        val MAX_VALUE = EnumSet.allOf(ResourceField::class.java).size()
        private val TYPE_VALUES = arrayOfNulls<ResourceField>(MAX_VALUE)

        init {
            var value = 0
            for (e in EnumSet.allOf(ResourceField::class.java)) {
                e.m_value = value++
