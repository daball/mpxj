 ...
 */
class FastTrackReader : ProjectReader {

    private var m_data: FastTrackData? = null
    ...
    private fun read(): ProjectFile {
        ...
        m_project!!.projectProperties.fileApplication = "FastTrack"
        m_project!!.projectProperties.fileType = "FTS"
    ...
    private fun processResources() {
        val table = m_data!!.getTable(FastTrackTableType.RESOURCES)
        for (row in table!!) {
            val uniqueID = row.getInt(ResourceField.RESOURCE_ID)
            if (uniqueID <= 0) {
                continue
            }

            val resource = m_project!!.addResource()
            resource.code = row.getString(ResourceField.CODE)
            resource.costPerUse = row.getCurrency(ResourceField.PER_USE_COST)
            resource.emailAddress = row.getString(ResourceField.EMAIL_ADDRESS)
            ...
            resource.group = row.getString(ResourceField.GROUP)
            resource.guid = row.getUUID(ResourceField._RESOURCE_GUID)
            resource.initials = row.getString(ResourceField.INITIALS)
            resource.materialLabel = row.getString(ResourceField.MATERIAL_LABEL)
            resource.name = row.getString(ResourceField.RESOURCE_NAME)
            resource.notes = row.getString(ResourceField.RESOURCE_NOTES)
            ...
            resource.uniqueID = Integer.valueOf(uniqueID)
    ...
    private fun processTasks() {
        ...
        val activities = m_data!!.getTable(FastTrackTableType.ACTIVITIES)
        for (row in activities!!) {
            ++uniqueID

            val id = row.getInteger(ActivityField.ACTIVITY_ROW_ID)
            if (id == null || id!!.intValue() < 1) {
                continue
            }

            val task = m_project!!.addTask()
            task.name = row.getString(ActivityField.ACTIVITY_NAME)
            task.id = id
            ...
            task.wbs = row.getString(ActivityField.WBS)
            task.guid = row.getUUID(ActivityField._ACTIVITY_GUID)
            task.outlineLevel = getOutlineLevel(task)
            m_eventManager!!.fireTaskReadEvent(task)
        }

        val table = m_data!!.getTable(FastTrackTableType.ACTBARS)
        val tasksWithBars = HashSet<Task>()
        ...
        for (row in table!!) {
            ...
            val task = m_project!!.getTaskByUniqueID(row.getInteger(ActBarField._ACTIVITY))
            ...
            // % Used
            task.actualDuration = row.getDuration(ActBarField.ACTUAL_DURATION)
            task.actualFinish = row.getTimestamp(ActBarField.ACTUAL_FINISH_DATE, ActBarField.ACTUAL_FINISH_TIME)
            task.actualStart = row.getTimestamp(ActBarField.ACTUAL_START_DATE, ActBarField.ACTUAL_START_TIME)
            // Attendees
            ...
            task.constraintDate = row.getTimestamp(ActBarField.CONSTRAINT_DATE, ActBarField.CONSTRAINT_TIME)
            ...
            // Created
            task.critical = row.getBoolean(ActBarField.CRITICAL)
            ...
            task.duration = row.getDuration(ActBarField.DURATION)
            ...
            task.earlyFinish = row.getTimestamp(ActBarField.EARLY_FINISH_DATE, ActBarField.EARLY_FINISH_TIME)
            task.earlyStart = row.getTimestamp(ActBarField.EARLY_START_DATE, ActBarField.EARLY_START_TIME)
            task.effortDriven = row.getBoolean(ActBarField.EFFORT_DRIVEN)
            task.finish = row.getTimestamp(ActBarField.FINISH_DATE, ActBarField.FINISH_TIME)
            ...
            // Fixed Duration
            // Free Float
            task.ignoreResourceCalendar = row.getBoolean(ActBarField.IGNORE_RESOURCE_CALENDARS)
            task.lateFinish = row.getTimestamp(ActBarField.LATE_FINISH_DATE, ActBarField.LATE_FINISH_TIME)
            task.lateStart = row.getTimestamp(ActBarField.LATE_START_DATE, ActBarField.LATE_START_TIME)
            ...
            task.percentageComplete = row.getDouble(ActBarField.PERCENT_COMPLETE)
            // Priority
            // Resource Cost
            task.resourceNames = row.getString(ActBarField.RESOURCES_ASSIGNED)
            // Revised Duration
            // Revised Finish Date
            // Revised Finish Time
            // Revised Start Date
            // Revised Start Time
            task.start = row.getTimestamp(ActBarField.START_DATE, ActBarField.START_TIME)
            // Start Float
            ...
            task.work = row.getWork(ActBarField.WORK)
            // _Activity
            // _BarBits
            // _BarStl
            // _yOffset
    ...
    private fun processDependencies() {
        ...
        for (row in table!!) {
            ...
            val predecessors = row.getString(ActBarField.PREDECESSORS)
            if (predecessors == null || predecessors!!.isEmpty()) {
                continue
            }

            for (predecessor in predecessors!!.split(", ")) {
                val matcher = RELATION_REGEX.matcher(predecessor)
                matcher.matches()

                val id = Integer.valueOf(matcher.group(1))
                var type = RELATION_TYPE_MAP.get(matcher.group(3))
                if (type == null) {
                    type = RelationType.FINISH_START
                }

                val sign = matcher.group(4)
                var lag = NumberHelper.getDouble(matcher.group(5))
                if ("-".equals(sign)) {
                    lag = -lag
                }

                val targetTask = m_project!!.getTaskByID(id)
                if (targetTask != null) {
                    val lagDuration = Duration.getInstance(lag, m_data!!.durationTimeUnit)
                    val relation = task.addPredecessor(targetTask, type, lagDuration)
    ...
    private fun processAssignments() {
        ...
        for (row in table!!) {
            ...
            val assignments = row.getString(ActBarField.RESOURCES_ASSIGNED)
            ...
            for (assignment in assignments!!.split(", ")) {
                ...
                val matcher = ASSIGNMENT_REGEX.matcher(assignment)
                matcher.matches()

                val resource = resources.get(matcher.group(1))
                if (resource != null) {
                    val ra = task.addResourceAssignment(resource)
                    val units = matcher.group(2)
                    if (units != null) {
                        ra.units = Integer.valueOf(units)
                    }
     ...
     */
    private fun getOutlineLevel(task: Task): Integer {
        val value = task.wbs
        var result = Integer.valueOf(1)
        if (value != null && value.length() > 0) {
            val path = WBS_SPLIT_REGEX.split(value)
            result = Integer.valueOf(path.size)
        }
        return result
    }
    ...
    companion object {
        ...
        private val WBS_SPLIT_REGEX = Pattern.compile("(\\.|\\-|\\+|\\/|\\,|\\:|\\;|\\~|\\\\|\\| )")
        private val RELATION_REGEX = Pattern.compile("(\\d+)(:\\d+)?(FS|SF|SS|FF)*(\\-|\\+)*(\\d+\\.\\d+)*")
        private val ASSIGNMENT_REGEX = Pattern.compile("([^\\[]+)(?:(?:\\[(-?\\d+)\\%\\])|(?:\\[.+\\]))?")
