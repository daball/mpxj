 ...
 */
class MPXJDateFormat : MPXJBaseFormat() {
    /**
     ...
     */
    init {
        m_formats = arrayOf<SimpleDateFormat>(SimpleDateFormat("dd/MM/yyyy", Locale.ENGLISH))
    }
    ...
    fun setLocale(locale: Locale) {
        val formats = ArrayList<SimpleDateFormat>()
        for (format in m_formats) {
            formats.add(SimpleDateFormat(format.toPattern(), locale))
        }

        m_formats = formats.toArray(arrayOfNulls<SimpleDateFormat>(formats.size()))
    }
}
