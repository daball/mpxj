 ...
 */
enum class ProjectField// Must always be last value
 ...
 */
private constructor(private val m_dataType: DataType,
                    /**
                     * {@inheritDoc}
                     */
                    @get:Override val unitsType: FieldType? = null) : FieldType {
    ...
    val fieldTypeClass: FieldTypeClass
        @Override get() = FieldTypeClass.PROJECT
    ...
    fun getName(locale: Locale): String? {
        val titles = LocaleData.getStringArray(locale, LocaleData.PROJECT_COLUMNS)
    ...
    companion object {
         ...
         */
        fun getInstance(type: Int): ProjectField? {
            var result: ProjectField? = null
        ...
        val MAX_VALUE = EnumSet.allOf(ProjectField::class.java).size()
        private val TYPE_VALUES = arrayOfNulls<ProjectField>(MAX_VALUE)

        init {
            var value = 0
            for (e in EnumSet.allOf(ProjectField::class.java)) {
                e.m_value = value++
